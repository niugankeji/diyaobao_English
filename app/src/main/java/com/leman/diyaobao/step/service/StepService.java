package com.leman.diyaobao.step.service;
...
import com.leman.diyaobao.step.accelerometer.StepCount;
import com.leman.diyaobao.step.accelerometer.StepValuePassListener;
...
public class StepService extends Service implements SensorEventListener {
    ...
    private void initNotification() {
        ...
        mBuilder.setContentTitle(getResources().getString(R.string.app_name))
                .setContentText("The number of steps to walk today:" + CURRENT_STEP )
...
//                .setSmallIcon(R.mipmap.logo);
    ...
    private void initTodayData() {
        ...
        RealmResults<StepData> results = realm.where(StepData.class)
                .equalTo("today", CURRENT_DATE)
                .findAll();
        if (results.size() == 0) {
            CURRENT_STEP = 0;
        } else if (results.size() == 1) {
            Log.v(TAG, "StepData=" + results.get(0).toString());
            CURRENT_STEP = results.get(0).getStep();
        } else {
            Log.v(TAG, "Error！");
        }
        if (mStepCount != null) {
...
//        mNotificationManager.notify(notifyId_Step, notification);
    ...
    private void remindNotify() {
        ...
        mBuilder.setContentTitle("The number of steps to walk today" + CURRENT_STEP)
                .setContentText("There is " + (Integer.valueOf(plan) - CURRENT_STEP) + "steps from the target！")
                .setContentIntent(hangPendingIntent)
                .setTicker(getResources().getString(R.string.app_name) + "Remind you to start exercising")//通知首次出现在通知栏，带上升动画效果的
                .setWhen(System.currentTimeMillis())//通知产生的时间，会在通知信息里显示
                .setPriority(Notification.PRIORITY_DEFAULT)//设置该通知优先级
...
//                .setSmallIcon(R.mipmap.logo);
    ...
    public void onSensorChanged(SensorEvent event) {
        if (stepSensorType == Sensor.TYPE_STEP_COUNTER) {
            ...
            LogUtil.d(TAG, "tempStep" + tempStep);
    ...
    private void addBasePedometerListener() {
        ...
        if (isAvailable) {
            Log.v(TAG, "The acceleration sensor can be used");
        } else {
            Log.v(TAG, "Acceleration sensor not available");
        }
    }
...
//        List<StepData> list = DbUtils.getQueryByWhere(StepData.class, "today", new String[]{CURRENT_DATE});
        ...
        Realm realm = Realm.getDefaultInstance();
        final RealmResults<StepData> results = realm.where(StepData.class)
                .equalTo("today", CURRENT_DATE)
                .findAll();
        ...
        if (results.size() == 0) {
            ...
            realm.executeTransaction(new Realm.Transaction() {
                ...
                public void execute(Realm realm) {
                    //自动更新
                    final StepData data = results.get(0);
