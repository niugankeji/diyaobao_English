...
//        styledAttrs.recycle();
    ...
    protected void deliverAndDrawFrame(CvCameraViewFrame frame) {
		...
		}else if (mCacheBitmap.getWidth() != modified.width() || mCacheBitmap.getHeight() != modified.height()) {
			mCacheBitmap.recycle();
			mCacheBitmap = Bitmap.createBitmap(modified.width(), modified.height(), Bitmap.Config.ARGB_8888);
		}     
        /*************************/
    ...
    protected Size calculateCameraFrameSize(List<?> supportedSizes, ListItemAccessor accessor, int surfaceWidth, int surfaceHeight) {
        ...
        //Get the phone's manufacture
        String PhoneManufacturer = Build.MANUFACTURER;
        
        if (PhoneManufacturer.equalsIgnoreCase("Meizu") || PhoneManufacturer.equalsIgnoreCase("Huawei")) {
        	calcWidth  = Integer.MAX_VALUE;
        	calcHeight = Integer.MAX_VALUE;
        	for (Object size : supportedSizes) {
                int width  = accessor.getWidth(size);
                int height = accessor.getHeight(size);
                if(width*3 == height*4 && height>=640)
                {
		...
		}else if(PhoneManufacturer.equalsIgnoreCase("Xiaomi")){
			for (Object size : supportedSizes) {
	            int width = accessor.getWidth(size);
	            int height = accessor.getHeight(size);
